...
class ControllerMapperPreferences(AddonPreferences):
    ...
    def draw(self, context):
        ...
        if not XINPUT_AVAILABLE:
            ...
            box.label(text="XInput: ✅ Ready", icon='CHECKMARK')

        # Test button
        row = layout.row(align=True)
        row.operator("wm.controller_test_xinput", text="Test XInput Connection")
...
class WM_OT_controller_test_xinput(Operator):
    bl_idname = "wm.controller_test_xinput"
    bl_label = "Test XInput"

    def execute(self, context):
        if not XINPUT_AVAILABLE:
            self.report({'ERROR'}, "XInput not available")
            return {'CANCELLED'}

        state = get_xinput_state()
        if not state:
            self.report({'WARNING'}, "No controller detected (index 0)")
            print("❌ No controller found on index 0")
            return {'CANCELLED'}

        gp = state.Gamepad
        print(f"✅ Controller connected!")
        print(f"Buttons: {gp.wButtons:#06x}")
        print(f"Left Stick: ({gp.sThumbLX}, {gp.sThumbLY})")
        print(f"Right Stick: ({gp.sThumbRX}, {gp.sThumbRY})")
        print(f"L/R Triggers: {gp.bLeftTrigger}/{gp.bRightTrigger}")

        self.report({'INFO'}, "Controller detected! See system console.")
        return {'FINISHED'}
...
class WM_OT_toggle_controller_mode(Operator):
    ...
    def modal(self, context, event):
        ...
        # Poll XInput state every frame
        try:
            state = get_xinput_state() if XINPUT_AVAILABLE else None
            if not state:
            ...
            # Debug: Print raw data once per second
            frame = context.scene.frame_current
            fps = context.scene.render.fps or 24
            if frame % int(fps) == 0:
                print(f"🎮 [Frame {frame}] Buttons={gp.wButtons:#06x}, LStick=({gp.sThumbLX}, {gp.sThumbLY}), RStick=({gp.sThumbRX}, {gp.sThumbRY})")
        ...
        # Generate simulated events for combo logic (buttons + sticks)
        now = context.scene.frame_current / (context.scene.render.fps or 24)

        # Simulate button press release based on wButtons bitmask
        btns = [
        ...
        for code, mask in btns:
            ...
            if pressed and code not in self.pressed_keys:
                self.pressed_keys.add(code)
            elif not pressed and code in self.pressed_keys:
                self.pressed_keys.discard(code)

        # Simulate stick events (only when active > deadzone)
        ...
        for code, val in [
            ...
            if abs(val) > deadzone:
                self.pressed_keys.add(code)
            elif code in self.pressed_keys:
                self.pressed_keys.discard(code)

        # Check combos (same logic as before)
    ...
    def execute(self, context):
        ...
        try:
            state = get_xinput_state()
            if not state:
                self.report({'WARNING'}, "Controller not detected. Check USB/connection.")
                print("❌ Controller not found on index 0")
                return {'CANCELLED'}
        except Exception as e:
            self.report({'ERROR'}, f"Controller error: {e}")
            return {'CANCELLED'}
...
class VIEW3D_PT_controller_mapper(Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        if XINPUT_AVAILABLE:
            row.label(text="✅ XInput Ready")
        else:
            row.alert = True
            row.label(text="❌ XInput Not Available", icon='ERROR')

        # Test button
        row.operator("wm.controller_test_xinput", text="", icon='PLAY')
